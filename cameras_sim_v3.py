...
# CAMERAS SIMULATION. This script sends all frames of 8 cameras at the "same time"
# to the broker under MQTT - TOPIC 'cameraframes'
...
# Function to process a all frames  name of a frame cameraName_type(color/depth)_numframe.png
# frame by frame and in order (sorted)
def process_frames_of_a_camera(client, k_dict, camera_name_path, dataset_id): 
    camera_name = os.path.basename(camera_name_path)
    logger.info(f"Sending all frames of camera {camera_name} INIT")
    color_frames = sorted(f for f in os.listdir(camera_name_path) if f.startswith(f"{camera_name}_color_"))
    depth_frames = sorted(f for f in os.listdir(camera_name_path) if f.startswith(f"{camera_name}_depth_"))
    ...
    for chosen_color_frame, chosen_depth_frame in zip(color_frames, depth_frames):
        encoded_color_file = encode_png_to_base64(os.path.join(camera_name_path, chosen_color_frame))
        encoded_depth_file = encode_png_to_base64(os.path.join(camera_name_path, chosen_depth_frame))
...
# nota: base_directory = data/first8_frames
def start_cam_simulation(client, base_directory, dataset_id, send_freq = 3):
    ...
    try:
        while not exit_sim:
            ...
            # data/first8frames/camera_names_dirs
            threads = []  
...
# MAIN
# 1 argument: path to the main directory which contains all images (RGB/DEPTH) and metadata
# data/first8_frames 
if __name__ == "__main__":
    parser = ArgumentParser()
    parser.add_argument('main_directory', help = 'Path to all files of the cameras')
    ...
    try:
        ...
        base_directory = args.main_directory
        dataset_id = 1
        x = input("Press ENTER ") 
