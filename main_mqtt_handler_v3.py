...
def create_pc_from_data(color_image_path, depth_image_path, K, target_ds, color_frame_name):
    #try:
    #    color_image = cv2.imread(color_image_path)
    #    depth_image = cv2.imread(depth_image_path)
    #    print(f"LOG: depth frame sigue siendo uint16: {depth_image.dtype}")
    #    depth_image=depth_image.astype("uint16")
    #    print(f"LOG despues de convertir: depth frame sigue siendo uint16: {depth_image.dtype}")
    #except Exception as e:
    #    logger.error(f"Error loading image: {e}   || len depth_image_data: {len(color_image)}  len color: {len(depth_image)}\n")
    #    return None
    
    #logger.debug(f"Color image shape: {color_image.shape}, dtype: {color_image.dtype}")
    ...
    #name = f"framea_{os.path.basename(color_frame_name).split('.')[0]}.ply"
    #save_and_upload_pcd(pcd, name)
...
def process_frames(msg_frames_list, num_frame):
    ...
    for message in msg_frames_list:
        ...
        pc = create_pc_from_data(enc_c, enc_d, K, target_ds, color_frame_name)
        pcd_list.append(pc)
        if pc is None:
        ...
        # delete frames
        os.remove(path_color_frame) # rgb original
        os.remove(path_depth_frame) # depth 
        os.remove(new_rgb_name_frame) # binary mask 
...
def process_message(msg):
    ...
    logger.info(f"[TS] Received MSG with frame {num_frame} of the camera {camera_name}")
